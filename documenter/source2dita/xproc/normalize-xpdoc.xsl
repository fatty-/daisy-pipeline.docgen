...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc>
        <xd:short>Matches each p:documentation element and normalizes them.</xd:short>
        <xd:detail>Determines what code is being documented and calls appropriate templates for
            short description, detailed description, copyright etc.</xd:detail>
        <xd:return>The normalized p:documentation.</xd:return>
    </xd:doc>
        ...
        <xd:detail>
            <ul>
                <li>If there is no &lt;xd:short/&gt;-element present, but plain text is present as a
                    child of the &lt;p:documentation/&gt;element, then the first sentence from that
                    text (up until the first period (".") will be used as a short description.</li>
    ...
    <xd:doc>
        ...
        <xd:detail>The detailed description will be put together based on any number of
            ...
            children of the &lt;p:documentation/&gt;-element. If the xd:short was extracted from the
            first text node, then the first sentence from that text node will be ignored. If there
            are multiple &lt;xd:detail/&gt; and text nodes, then each of them will be wrapped in
    ...
    <xd:doc>Normalizes xd:param elements. Will create empty documentation for imports that are not
        documented, and removes documentation of imports that does not exist in the code.</xd:doc>
    <xsl:template name="param">
        ...
        <xsl:for-each select="$code/p:param">
    ...
    <xd:doc>Normalizes xd:option elements. Will create empty documentation for imports that are not
        documented, and removes documentation of imports that does not exist in the code.</xd:doc>
    <xsl:template name="option">
        <xsl:param name="code" required="yes"/>
        <xsl:variable name="doc" select="."/>
        <xsl:for-each select="*[local-name()='option']">
            <xd:option>
                <xsl:apply-templates select="@*"/>
                <xsl:apply-templates mode="section" select="node()"/>
            </xd:option>
        </xsl:for-each>
        <xsl:for-each select="$code/p:option">
            <xsl:variable name="name" select="@name"/>
            <xsl:if test="not($doc/*[local-name()='option' and @name=$name])">
                <xd:option name="{@name}"/>
            </xsl:if>
        </xsl:for-each>
    </xsl:template>

    <xd:doc>Normalizes xd:input elements. Will create empty documentation for imports that are not
        documented, and removes documentation of imports that does not exist in the code.</xd:doc>
    <xsl:template name="input">
        ...
        <xsl:for-each select="$code/p:input">
            ...
            <xsl:if test="not($doc/*[local-name()='input' and @port=$port])">
                <xd:input name="{@port}"/>
            </xsl:if>
        </xsl:for-each>
    </xsl:template>

    <xd:doc>Normalizes xd:output elements. Will create empty documentation for imports that are not
        documented, and removes documentation of imports that does not exist in the code.</xd:doc>
    <xsl:template name="output">
        ...
        <xsl:for-each select="$code/p:output">
            <xsl:variable name="port" select="@port"/>
            <xsl:if test="not($doc/*[local-name()='output' and @port=$port])">
                <xd:output name="{@port}"/>
            </xsl:if>
        </xsl:for-each>
    </xsl:template>
